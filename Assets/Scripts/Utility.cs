...
public static class Utility {
    ...
    public static T RandomItem<T>(this IList<T> list, int startRank = 0)
        => RandomItem(list, DefaultRandom, startRank);

    public static T RandomItem<T>(this IList<T> list, Random random, int startRank = 0)
        => list[random.Next(startRank, list.Count)];

    public static T RandomValue<T>(int startRank = 0) where T : IConvertible, IFormattable, IComparable
        => ((T[])Enum.GetValues(typeof(T))).RandomItem(startRank);
