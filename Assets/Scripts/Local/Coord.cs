...
public struct Coord : IComparable<Coord> {
    ...
    public Coord Normalize => new(x.Sign(), y.Sign(), z.Sign());
    public Coord Abs => new(x.Abs(), y.Abs(), z.Abs());
    ...
    public Coord[] GetAdjacent(bool includeSelf, bool diagonal) {
        ...
        if (diagonal)
            coords.AddRange(new[]
                { this + Right + Forward, this + Left + Forward, this + Right + Back, this + Left + Back });
    ...
    public static readonly Coord One = new(1, 1, 1);
    ...
    public static readonly Coord Back = new(0, 0, -1);
    ...
    public Coord(Vector3 vector) : this(Mathf.RoundToInt(vector.x), Mathf.RoundToInt(vector.y),
    ...
    public static implicit operator Vector3(Coord a) => new(a.x, a.y, a.z);
    ...
    public static Coord operator *(int i, Coord c) => new(c.x * i, c.y * i, c.z * i);
    public static Coord operator *(Coord c, int i) => i * c;

    public static Coord operator /(Coord c, int i) => new(c.x / i, c.y / i, c.z / i);

    public static Coord operator -(Coord c) => new(-c.x, -c.y, -c.z);

    public static Coord operator *(QuaternionInt q, Coord c) => new(q.ToQuaternion() * c);
