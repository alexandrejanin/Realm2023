...
namespace UnityEditor.PostProcessing
{
    ...
    public class ColorGradingModelEditor : PostProcessingModelEditor
    {
		...
		float NeutralCurve(float x, float a, float b, float c, float d, float e, float f) => ((x * (a * x + c * b) + d * e) / (x * (a * x + b) + d * f)) - e / f;
        ...
        void DoColorWheelsGUI()
        {
	        ...
	        {
		        GUILayout.Space(15);
		        if (GUILayout.Toggle(wheelMode == (int)ColorGradingModel.ColorWheelMode.Linear, "Linear", EditorStyles.miniButtonLeft)) wheelMode = (int)ColorGradingModel.ColorWheelMode.Linear;
		        if (GUILayout.Toggle(wheelMode == (int)ColorGradingModel.ColorWheelMode.Log, "Log", EditorStyles.miniButtonRight)) wheelMode = (int)ColorGradingModel.ColorWheelMode.Log;
	        }
